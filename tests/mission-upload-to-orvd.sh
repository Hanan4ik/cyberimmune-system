...
echo "state is: "$(curl -s "http://$HOST:$PORT/admin/get_state?id=inspector&token=$TOKEN")

# get mission
# curl "http://$HOST:$PORT/admin/get_mission?id=inspector&token=$TOKEN"
# $-1

# get mission state
# curl "http://$HOST:$PORT/admin/get_mission_state?id=inspector&token=$TOKEN"
# $-1
...
echo "mission upload key is: " $(curl -s "http://$HOST:$PORT/mission_sender/key?id=inspector")
...
# mission acknowledge
# example answer: $OK
echo "mission acknowledge: " $(curl -s "http://$HOST:$PORT/admin/mission_decision?id=inspector&decision=0&token=$TOKEN")
...
echo "get mission: " $(curl -s "http://$HOST:$PORT/admin/get_mission?id=inspector&token=$TOKEN")
