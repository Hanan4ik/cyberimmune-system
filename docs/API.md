...
Возвращает значение DOP (снижение точности) и число наблюдаемых спутников (sats). При использовании локальной навигации всегда возвращаются значения 1.0 и 8.

#### `int getEstimatedSpeed(float& speed)`

Возвращает скорость (в м/с), определенную модулем GNSS. При использовании локальной навигации скорость не оценивается, и функция всегда возвращает значение 0.
...
Делает несколько попыток прочесть RFID-метку. Останавливается, когда находит метку, или когда заканчивается установленный лимит попыток. Была ли метка прочитана или нет, возвращается в scanResult.
