...
2. Выполняются только авторизованные системой ИОрВД задания, при этом статус авторизации может быть изменён в процессе выполнения задания.
3. Все операции, включая место сброса груза, выполняются согласно ограничениям в полётном задании.
4. В случае критического отказа дрон снижается вертикально со скоростью не более 1 м/с с включенным звуковым сигналом, имитирующим функцию открытия парашюта.
5. Для запроса авторизации вылета к системе ИОрВД используется только аутентичный идентификатор дрона.
6. В рассчётах систем безопасности используются только аутентичные данные от других дронов.
